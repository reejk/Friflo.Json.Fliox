...
public static class Test_ChunkEntities
{
    ...
    {
        var store = Test_Query.SetupTestStore();
        ...
        foreach (var (position, entities) in query.Chunks)
        {
            switch (chunkCount++) { 
                case 0:
                    Mem.AreEqual("Entity[1]    Archetype: [MyComponent1, EntityName, Position, Rotation, Scale3, Transform]  entities: 1",   entities.ToString());
